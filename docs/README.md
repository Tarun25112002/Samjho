...
**Vision.** Not a question bank with an exam mode bolted on. A loop: practise → get it wrong → understand *why* → re-practise that. The mistake is a first-class object in the schema, not a log row.
...
2. **An exam position is a *slot* that can hold alternatives** — internal choice ("attempt either/or") is everywhere in real papers.
