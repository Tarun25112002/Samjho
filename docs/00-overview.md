...
- Every wrong attempt is tagged with a *reason* (conceptual gap / calculation slip / misread question / ran out of time / didn't attempt), partly by the student, partly inferred.
...
- Often phone-first. A meaningful share have *no* personal laptop, which affects the exam simulator's mobile posture.
...
- Age 16–18, preparing for boards *and usually* JEE/NEET simultaneously. Higher willingness to pay.
...
Journeys are written as *goals*, with the page-level flow in `01-information-architecture.md`.
...
> Design note: an empty dashboard on day one is the single biggest driver of churn in study apps. The onboarding must end *inside* a practice session, not on a dashboard.
...
**Content scope for MVP:** **Class 10 Mathematics and Class 10 Science.** *(Decided 2026-08-08.)*
...
**Content reality:** the bank is being built from zero. That makes admin throughput the critical path, and it reorders the phase plan (see `07-roadmap-risks-questions.md` §1) — admin content tooling now ships *before* practice mode so content entry can begin roughly a week earlier and run in parallel with the rest of the build.
...
| Mistake-repair rate: % of missed questions re-attempted correctly within 14 days | The *differentiating* loop is running | > 45% |
...
2. An exam position can offer **alternative questions** → the paper is a list of *slots*, each holding one or more question options.
